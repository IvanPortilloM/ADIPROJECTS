...
- **DsCA** (9; EN CURSO): ✅ frmDetProducto (4846b1d), ✅ frmDetCredito (c271e0d; patrón SP-39-OUTPUTs → diccionario con DynamicParameters), ✅ frmCarnetImp (7a8fcaf). Faltan: IA\Busquedas\frmBuscarAsociados (113), SAC\frmMenu (222), Herramientas\frmDevoluciones (311), IA\Transaccionales\frmInformacionAsoc (958, también usa DsCodeasAdiggm), SAC\FrmSolCred (1926) + VarGlobales.consultasCA.
...
## 13. TAREA INMEDIATA
Continuar **DsCA** con `RepositorioCA` (ya existe): siguiente `IA\Busquedas\frmBuscarAsociados`, luego SAC\frmMenu, frmDevoluciones, frmInformacionAsoc (parte CA), FrmSolCred; al final reemplazar `VarGlobales.consultasCA` y borrar `DsCA.xsd`. UN form por turno; **análisis de errores de lógica en cada form (§0)**, los GRAVES se reparan en la migración; build verde + commit + actualizar §6/§9/§12/§13. **Gotcha §11 en todo Designer con grid.**
