...
## 13. TAREA INMEDIATA
Siguiente DataSet según §9: **DsFAC** (11 forms de facturación) o seguir achicando **DsCodeasAdiggm** (RepositorioCodeas ya cubre buena parte; quedan ~12 forms). Sugerido: DsFAC primero (orden menos→más del roadmap). Crear `RepositorioFAC : RepositorioBase` (ver ConnectionRef del DsFAC.xsd). UN form por turno; **análisis de errores de lógica en cada form (§0)**, los GRAVES se reparan en la migración; build verde + commit + actualizar §6/§9/§12/§13. **Gotcha §11 en todo Designer con grid.**
