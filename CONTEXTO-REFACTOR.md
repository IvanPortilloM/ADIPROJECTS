...
- ⚠️ **GRIDS Y EL DISEÑADOR DE VS** (causa raíz hallada 2026-06-10): si el `.Designer.cs` deja `dgv.DataSource = <BindingSource>` y el BindingSource ya NO tiene DataSet de diseño (migrado), al abrir el form en el diseñador VS no puede resolver el esquema y ELIMINA las `DataGridViewColumn` EN DISCO (deja `AutoGenerateColumns=false` + 0 columnas → grid VACÍO en runtime, pero COMPILA; también toca el .resx). RECURRENTE hasta quitar el binding de diseño. **Patrón obligatorio al migrar forms con grid: QUITAR `dgv.DataSource = bs` del Designer y asignarlo en código en CargarX() después de `bs.DataSource = _dt`** (hecho en frmTipoOp/frmBodegas, commit 4ce41bc). Tras tocar un `.Designer.cs` de grid, verifica `dgv.Columns.AddRange(...)`, bloques `// <col>` con `DataPropertyName` y campos de columna; si se perdieron, restaura de git (`git show HEAD:<ruta>`). Las columnas usan `DataPropertyName` = nombre de columna del DataTable; el grid se habilita con `dgv.ReadOnly=false` (cascada a columnas).
