...
## 13. TAREA INMEDIATA
**DsFAC ✅ COMPLETO (914aad8).** Siguiente DataSet según §9 (orden menos→más): **DsOCWeb** (2 forms, enredado con dsOC/dsTransporteAdiggm — conviene junto con DsOC) o, más limpio para avanzar, **DsCodeasAdiggm** (15 forms, RepositorioCodeas ya cubre buena parte) o **DsPresupuesto** (25, muchos mantenimientos de catálogo simples → buen lote). Sugerido: empezar **DsPresupuesto** (lote de catálogos con patrón Opción A ya dominado) o seguir achicando **DsCodeasAdiggm**. Crear/extender el repo correspondiente (`RepositorioPresupuesto`/`RepositorioCodeas`) con `Conexion.PRESUPUESTO`/`Conexion.COVIBASE`/etc. según el ConnectionRef del .xsd. **Gotcha §8**: para retirar el DataSet por completo hay que reemplazar también `VarGlobales.consultasPR`/`consultas` (SP globales). UN form por turno; **análisis de errores de lógica en cada form (§0)**, los GRAVES se reparan en la migración; build verde + commit + actualizar §6/§9/§12/§13. **Gotchas §11 (grid sin DataSource de diseño) y bindings de diseño huérfanos en TODO Designer.** Patrones ya probados (reutilizar): Opción A (mant. grid editable), columna combo en grid, SP→grid+combo, SP con OUTPUT (`DynamicParameters` InputOutput), scalar SP (`Escalar<T>`).
