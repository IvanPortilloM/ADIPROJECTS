...
- **DsCodeasAdiggm** (15, EN CURSO — conexión TransporteAdiggm, `RepositorioCodeas` ya existe): ✅ Mant\FrmTipoFac (0f27b69, mant. TR_TipoFacturas). Pendientes: IA\frmInformacionAsoc, Mant\FrmAsigTpFacTpVeh/FrmCierreCliente/FrmCierresBuscar/FrmEstadosDeCuenta/FrmSyncTransCod, PRESUPUESTO\frmPresupuestoSem (⚠️ DIFERIDO — el usuario rehará Presupuesto; dejar para el final/no tocar aún), Reportes\RptMaestro, SAC\FrmAsocBuscar/FrmSolCred/frmVisorOrdenesSAC/frmOrdenes, Visores\FrmVisorPrestamos + VarGlobales.consultas. (frmInformacionAsoc y FrmSolCred ya migraron su parte de CA/Codeas en fases previas — confirmar con grep si aún tocan DsCodeasAdiggm.)
...
## 13. TAREA INMEDIATA
**DsFAC ✅ COMPLETO. DsPresupuesto DIFERIDO por el usuario (lo rehará).** En curso: achicar **DsCodeasAdiggm** (`RepositorioCodeas : RepositorioBase(Conexion.TRANSPORTE)` ya existe). ✅ Mant\FrmTipoFac (0f27b69). Siguiente sugerido: otro mantenimiento simple — **Mant\FrmAsigTpFacTpVeh** o **Mant\FrmCierreCliente/FrmCierresBuscar**; dejar IA\frmInformacionAsoc, SAC\frmOrdenes/frmVisorOrdenesSAC y FrmVisorPrestamos (más grandes) para después, y PRESUPUESTO\frmPresupuestoSem para nunca (diferido). **Gotcha §8**: para retirar DsCodeasAdiggm por completo hay que reemplazar también `VarGlobales.consultas` (SP globales de Codeas). UN form por turno; **análisis de errores de lógica (§0)**, GRAVES se reparan; build verde + commit + actualizar §6/§9/§12/§13. **Gotchas §11 (grid sin DataSource de diseño) y bindings de diseño huérfanos en TODO Designer.** Patrones probados (reutilizar): Opción A (mant. grid editable), columna combo en grid, SP→grid+combo, SP con OUTPUT (`DynamicParameters` InputOutput), scalar SP (`Escalar<T>`).
